...
@section('mainbody')
    ...
    <DIV>
        <FORM onsubmit="return check()" method=post enctype="multipart/form-data"
            name=postForm  action= "/reupdatedb/{{ $id }}/{{ $bid }}" >
            {{ csrf_field() }} 
    ...
    <TR class=tr3>
        ...
        <TH>
            <INPUT style="PADDING-LEFT: 2px; FONT: 14px Tahoma" class=input tabIndex=1 size=60 name=title value=reply: readonly=readonly>
        </TH>
    </TR>
    ...
    <TR class=tr3>
        ...
        <TH colSpan=2>
            <DIV>
                <SPAN>
                    <TEXTAREA style="WIDTH: 500px" tabIndex=2 rows=20 cols=90 name=content>{{ $content }}</TEXTAREA>
                </SPAN> 
